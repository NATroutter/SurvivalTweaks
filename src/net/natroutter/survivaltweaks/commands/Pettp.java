...
public class Pettp extends Command {
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        ...
        if (args.length == 0) {
            ...
            boolean state = database.getBoolean(p, "PetTP");
            if (state) {
                database.save(p, "PetTP", false);
                p.sendMessage(lang.Prefix + "PetTP: §cDisabled");
            } else {
                database.save(p, "PetTP", true);
                p.sendMessage(lang.Prefix + "PetTP: §cEnabled");
            }
